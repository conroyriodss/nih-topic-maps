...
print("Creating enhanced visualization (no BQ lookup)...")

# Load data - already has domain labels
...
# Simple label generation from domain info
def get_topic_label(topic_id, domain_id, domain_label):
    """Generate topic label from domain context"""
    # Use domain name + topic number for now
    return f"{domain_label[:25]} #{topic_id}"

def get_subtopic_label(subtopic_id, topic_id):
    """Generate subtopic label"""
    return f"Subtopic {subtopic_id}"

# Compute centroids with simple labels
def compute_centroids_with_labels(df, group_col, label_col=None):
    centroids = []
    for group_id in sorted(df[group_col].unique()):
        group_df = df[df[group_col] == group_id]
        
        if label_col and group_col == 'domain':
            label = str(group_df[label_col].iloc[0])
        elif group_col == 'topic':
            # Use domain info for topic labels
            domain_id = group_df['domain'].iloc[0]
            domain_label = group_df['domain_label'].iloc[0]
            label = f"{domain_label[:20]} T{group_id}"
        else:  # subtopic
            topic_id = group_df['topic'].iloc[0]
            label = f"Subtopic {group_id}"
...
topic_centroids = compute_centroids_with_labels(df, 'topic')
subtopic_centroids = compute_centroids_with_labels(df, 'subtopic')
...
print(f"   {len(domain_boundaries)} domain boundaries")
...
with open('nih_topic_map_50k_enhanced.html', 'w', encoding='utf-8') as f:
    ...
    f.write('<title>NIH Topic Maps - Enhanced</title>\n')
    ...
    f.write('.cluster-label{font-size:12px;font-weight:600;fill:#1a1a1a;text-anchor:middle;pointer-events:none;text-shadow:0 0 4px white,0 0 4px white}\n')
    ...
    f.write('<h1>NIH Research Portfolio - 50K Grants (Enhanced)</h1>\n')
    f.write('<div class="subtitle">Zoom-responsive labels • Domain boundaries • Dynamic loading • Background map</div>\n')
    ...
    # JavaScript (same as before, just limit data to 20k)
    f.write('<script>\nconst allData = ')
...
print("\nEnhancements:")
print("  ✓ Labels include domain context (not just numbers)")
print("  ✓ Labels scale with zoom (stay readable)")
print("  ✓ Domain boundaries with semi-transparent lines")
print("  ✓ Background map (10k sample, always visible)")
print("  ✓ Dynamic point loading (more grants when zoomed)")
