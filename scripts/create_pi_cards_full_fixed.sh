...
echo "=========================================="
echo "CREATING FULL PI CARDS (FIXED)"
...
pi_institution_years AS (
  SELECT
    contact_pi_name,
    institution,
    fiscal_year,
    COUNT(DISTINCT CORE_PROJECT_NUM) as num_awards_that_year,
    SUM(annual_funding) as funding_that_year
  FROM year_expanded
  GROUP BY contact_pi_name, institution, fiscal_year
)
SELECT
  contact_pi_name,
  institution,
...
mpi_awards AS (
  SELECT DISTINCT
    ...
    SUM(annual_funding) as total_funding,
    ANY_VALUE(cluster_label) as cluster_label
  FROM year_expanded
  GROUP BY contact_pi_name, CORE_PROJECT_NUM, all_pi_names, ADMINISTERING_IC
)
SELECT
  ...
  -- Temporal
  MIN(first_year) as first_collaboration_year,
  MAX(last_year) as latest_collaboration_year,
  
  -- Funding
  ROUND(SUM(total_funding)/1e6, 2) as total_collaborative_funding_millions,
...
WITH year_expanded AS (
  SELECT
    ...
    LAST_FISCAL_YEAR
  FROM `od-cl-odss-conroyri-f75a.nih_analytics.grant_cards_v1_6_with_agency`,
  UNNEST(GENERATE_ARRAY(FIRST_FISCAL_YEAR, LEAST(LAST_FISCAL_YEAR, 2024))) as year_num
  WHERE contact_pi_name IS NOT NULL
    AND cluster_id IS NOT NULL
),
award_classifications AS (
  ...
  FROM year_expanded
  GROUP BY contact_pi_name, CORE_PROJECT_NUM, ACTIVITY, TOTAL_LIFETIME_FUNDING
)
SELECT
  contact_pi_name,
  ...
  MIN(CASE WHEN career_stage_indicator = 'Early Career' THEN first_year END) as first_early_career_award,
  MIN(CASE WHEN career_stage_indicator = 'Senior/Established' THEN first_year END) as first_senior_award,
...
# Step 4: PI Patents & Clinical Studies (Placeholder)
echo ""
...
echo "✓ Created pi_broader_impact"
...
echo ""
echo "Tables created:"
echo "  - pi_institutional_history"
echo "  - pi_collaborations"
echo "  - pi_award_portfolio"
echo "  - pi_broader_impact"
echo "  - pi_cards_complete"
echo ""
