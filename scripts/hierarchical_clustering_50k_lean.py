#!/usr/bin/env python3
"""
Hierarchical clustering for 50k - lean version (no intermediate saves)
"""
import pandas as pd
import numpy as np
...
print("=" * 80)
print("HIERARCHICAL CLUSTERING: 50K GRANTS (LEAN)")
print("=" * 80)
...
print(f"  Features: {science_features.shape}, {science_features.nbytes/1e9:.2f} GB")

# LEVEL 1: Domains
print(f"\n[4/5] Clustering into {K_DOMAINS} domains...")
...
for domain_id in sorted(df['domain'].unique()):
    ...
    print(f"  {domain_id:2d}. {manual_domain_labels[domain_id]:40s}: {count:,}")

# LEVEL 2: Topics
print(f"\n[5/5] Creating topics and subtopics...")
df['topic'] = 0
topic_counter = 1
...
for domain_id in sorted(df['domain'].unique()):
    ...
    k_topics = max(2, min(K_TOPICS_PER, len(domain_features) // 50))
...
for topic_id in sorted(df['topic'].unique()):
    ...
    k_subtopics = max(2, min(K_SUBTOPICS_PER, len(topic_features) // 20))
...
print("\n" + "=" * 80)
print(f"COMPLETE: {len(df):,} grants clustered")
...
# Clean up memory
del science_features, embeddings_scaled, rcdc_scaled, terms_scaled
print("\nNext: Refine labels and add UMAP")
