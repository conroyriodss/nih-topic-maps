...
echo "=========================================="
echo "CREATING FULL IC CARDS (FIXED)"
...
WITH award_classifications AS (
  SELECT
    *,
    -- Extract fiscal year from available fields
    -- Assuming we have FIRST_FISCAL_YEAR, LAST_FISCAL_YEAR, or similar
    CAST(SUBSTR(CAST(FIRST_FISCAL_YEAR AS STRING), 1, 4) AS INT64) as start_year,
    CAST(SUBSTR(CAST(LAST_FISCAL_YEAR AS STRING), 1, 4) AS INT64) as end_year,
...
-- Generate year series for each award (FIRST_FISCAL_YEAR to LAST_FISCAL_YEAR)
year_expanded AS (
  SELECT
    ADMINISTERING_IC,
    ...
    pi_count,
    publication_count,
    -- Approximate annual funding (total divided by years)
    TOTAL_LIFETIME_FUNDING / GREATEST(1, end_year - start_year + 1) as annual_funding
  FROM award_classifications,
  UNNEST(GENERATE_ARRAY(start_year, end_year)) as year_num
  WHERE year_num BETWEEN 1985 AND 2024
)
SELECT
  ADMINISTERING_IC,
  fiscal_year,
  ...
  ROUND(SUM(CASE WHEN award_category = 'Research Project Grants (RPG)' THEN annual_funding ELSE 0 END)/1e6, 2) as rpg_funding_m,
  
  COUNTIF(award_category = 'Program Projects & Centers') as center_awards,
  ROUND(SUM(CASE WHEN award_category = 'Program Projects & Centers' THEN annual_funding ELSE 0 END)/1e6, 2) as center_funding_m,
  
  COUNTIF(award_category = 'Fellowships & Career Awards') as fellowship_awards,
  ROUND(SUM(CASE WHEN award_category = 'Fellowships & Career Awards' THEN annual_funding ELSE 0 END)/1e6, 2) as fellowship_funding_m,
  
  COUNTIF(award_category = 'Training Grants') as training_awards,
  ROUND(SUM(CASE WHEN award_category = 'Training Grants' THEN annual_funding ELSE 0 END)/1e6, 2) as training_funding_m,
...
# Step 2: Create comprehensive IC cards (same as before)
echo ""
...
SELECT 
  ...
  total_unique_pis,
  total_institutions,
  CONCAT(CAST(earliest_year AS STRING), '-', CAST(latest_year AS STRING)) as active_period,
  rpg_total_awards,
  center_total_awards,
