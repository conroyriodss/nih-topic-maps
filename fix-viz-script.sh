#!/bin/bash
# Fix create-viz-heatmap-cards.py syntax errors
# Run this in ~/nih-topic-maps directory

echo "Fixing JavaScript syntax errors in create-viz-heatmap-cards.py..."

# Create the corrected version
cat > create-viz-heatmap-cards.py << 'EOFPYTHON'
import pandas as pd
import json
import numpy as np

print("Creating ENHANCED visualization with heatmaps and award cards...")

# Load data
df = pd.read_csv('hierarchical_50k_with_umap.csv')
print(f"Loaded {len(df):,} grants")

# Generate full descriptive labels
print("Generating full descriptive labels...")

def get_full_topic_label(topic_id, domain_label):
    """Generate full topic label without abbreviation"""
    return f"{domain_label} - Topic {topic_id}"

def get_full_subtopic_label(subtopic_id, topic_label):
    """Generate full subtopic label"""
    return f"{topic_label} - Subtopic {subtopic_id}"

# Compute centroids with FULL labels
def compute_centroids_with_full_labels(df, group_col):
    centroids = []
    for group_id in sorted(df[group_col].unique()):
        group_df = df[df[group_col] == group_id]
        
        if group_col == 'domain':
            label = str(group_df['domain_label'].iloc[0])
        elif group_col == 'topic':
            domain_label = group_df['domain_label'].iloc[0]
            label = get_full_topic_label(group_id, domain_label)
        else:  # subtopic
            topic_id = group_df['topic'].iloc[0]
            domain_label = group_df['domain_label'].iloc[0]
            topic_label = get_full_topic_label(topic_id, domain_label)
            label = get_full_subtopic_label(group_id, topic_label)
...
domain_centroids = compute_centroids_with_full_labels(df, 'domain')
topic_centroids = compute_centroids_with_full_labels(df, 'topic')
subtopic_centroids = compute_centroids_with_full_labels(df, 'subtopic')

print(f"   {len(domain_centroids)} domains")
print(f"   {len(topic_centroids)} topics")
print(f"   {len(subtopic_centroids)} subtopics")

# Compute heatmap grid for domains
print("Computing domain heatmap...")
...
# For each domain, create heatmap data
heatmap_data = []
for domain_id in sorted(df['domain'].unique()):
    ...
    if len(domain_df) >= 3:
        ...
        # Create density values (1 at grant locations, 0 elsewhere)
        values = np.ones(len(points))
        
        # Interpolate to grid
        try:
            grid_z = griddata(points, values, (xx, yy), method='cubic', fill_value=0)
            grid_z = np.maximum(grid_z, 0)  # No negative values
            
            # Store grid points with values > threshold
            for i in range(grid_resolution):
                for j in range(grid_resolution):
                    if grid_z[i, j] > 0.1:  # Threshold
                        heatmap_data.append({
...
# Prepare grant data with grouping by PI/Institution for award cards
print("Preparing award cards with grouping...")

# Group by PI name (approximated from title) and Institution
data_records = []

for idx, row in df.iterrows():
    data_records.append({
        'id': str(row['APPLICATION_ID']),
        'title': str(row['PROJECT_TITLE']),  # Full title, no truncation
        'ic': str(row['IC_NAME']),
        'fy': int(row['FY']),
...
print(f"   {len(data_records):,} grants")

# Write HTML
print("Creating HTML with heatmap and award cards...")

with open('nih_topic_map_50k_heatmap_enhanced.html', 'w', encoding='utf-8') as f:
    f.write('<!DOCTYPE html>\n<html>\n<head>\n<meta charset="utf-8">\n')
    f.write('<title>NIH Topic Maps - Heatmap & Award Cards</title>\n')
    ...
    f.write('.cluster-label{font-size:13px;font-weight:600;fill:#1a1a1a;text-anchor:middle;pointer-events:none;text-shadow:0 0 4px white,0 0 4px white,0 0 4px white}\n')
    ...
    f.write('.legend-items{display:grid;grid-template-columns:repeat(auto-fill,minmax(450px,1fr));gap:5px}\n')
    ...
    f.write('<h1>NIH Research Portfolio - 50K Grants (Heatmap + Award Cards)</h1>\n')
    f.write('<div class="subtitle">Domain heatmaps • Full labels • Award grouping • Click grants for details</div>\n')
    ...
    f.write(json.dumps(data_records[:20000]))
    f.write(';\n')
    ...
    f.write(';\n')
    f.write('const topicCentroids = ')
    f.write(json.dumps(topic_centroids))
    f.write(';\n')
    f.write('const subtopicCentroids = ')
    f.write(json.dumps(subtopic_centroids))
    f.write(';\n\n')
    
    # JavaScript - properly formatted
    f.write('''let viewMode="domain",selectedClusters=new Set(),filteredData=allData,currentZoom=1;
...
heatmapData.forEach(cell=>{
    heatmapLayer.append("rect")
        .attr("x",xScale(cell.x)-10)
        .attr("y",yScale(cell.y)-10)
        .attr("width",20)
        .attr("height",20)
...
function showAwardCard(grant){
    const nearby=filteredData.filter(d=>
        Math.abs(d.x-grant.x)<0.5&&Math.abs(d.y-grant.y)<0.5
    ).slice(0,5);
    ...
    html+='<div class="award-card-header">Award Details ('+nearby.length+' grant'+(nearby.length>1?'s':'')+')</div>';
    
    nearby.forEach(d=>{
        html+='<div class="award-item">';
        html+='<div class="award-title">'+d.title+'</div>';
        html+='<div class="award-meta">';
        html+='<div>IC: '+d.ic+'</div>';
        html+='<div>FY: '+d.fy+'</div>';
        html+='<div>Funding: $'+(d.funding/1e6).toFixed(2)+'M</div>';
        html+='<div>ID: '+d.id+'</div>';
        html+='</div></div>';
    });
    
    html+='<div class="award-total">Total Funding: $'+(totalFunding/1e6).toFixed(2)+'M</div>';
    html+='</div>';
    
    document.getElementById("award-cards").innerHTML=html;
}

function closeAwardCard(){
...
function updateLabels(){
    ...
    labels.enter().append("text").attr("class","cluster-label").merge(labels)
        ...
        .attr("font-size",13/currentZoom+"px")
        .text(d=>d.label);
}
...
print("\n✅ Created: nih_topic_map_50k_heatmap_enhanced.html")
print("\nEnhancements:")
print("  ✓ Domain heatmaps (instead of boundaries)")
print("  ✓ FULL topic labels (no abbreviation)")
print("  ✓ FULL subtopic labels displayed")
print("  ✓ Award cards with multi-award grouping")
print("  ✓ Total budget shown for grouped awards")
print("  ✓ Full title text (word wrap, no cutoff)")
print("  ✓ Click any grant to see award card")
print("\nOpen nih_topic_map_50k_heatmap_enhanced.html in your browser!")
EOFPYTHON

echo ""
echo "✅ Fixed create-viz-heatmap-cards.py"
echo ""
echo "Now run:"
echo "  python3 create-viz-heatmap-cards.py"
echo ""
